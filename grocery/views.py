...
def Add_Product(request):
    ...
    if request.method == "POST":
        print("POST Data:", request.POST)  # Debugging: See received data
        print("FILES Data:", request.FILES)  # Debugging: See uploaded files

        c = request.POST.get('cat')  # Get category safely
        p = request.POST.get('pname')
        pr = request.POST.get('price')
        i = request.FILES.get('img')  # Avoid errors if file is missing
        d = request.POST.get('desc')

        if c and p and pr and i and d:
            try:
                ct = Category.objects.get(name=c)  # Get category from DB
                Product.objects.create(category=ct, name=p, price=pr, image=i, desc=d)
                error = True
            except Category.DoesNotExist:
                print("Error: Category does not exist!")  # Debugging
                error = "Selected category does not exist."

    return render(request, 'add_product.html', {'cat': categories, 'error': error})
...
def Add_Cart(request,pid):
    ...
    return HttpResponse(f"Product {pid} added to cart")
...
def profile(request):
    ...
    pro = Profile.objects.get(user=request.user)  # ✅ Correct
    cart = Cart.objects.filter(profile=pro)  # ✅ Queryset (if needed)

    d = {'pro': pro, 'cart': cart}  # ✅ Now passing the correct `pro`
...
class EditStatus(View ):
    def get(self, request, pid, bid):
        ...
        return render(request, 'edit_status.html', {'pid': pid, 'bid': bid})